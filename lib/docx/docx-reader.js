...
var metadataReader = require("./metadata-reader");
...
function read(docxFile, input) {
    ...
    return promises.props({
        ...
        return promises.extend(result, {
            coreProperties: readMetadataFromZipFile(docxFile, result.partPaths.coreProps),
            appProperties: readMetadataFromZipFile(docxFile, result.partPaths.appProps)
        });
    }).then(function(result) {
        return promises.extend(result, {
        ...
        var metadata = Object.assign(
            result.coreProperties.getMetadata(),
            result.appProperties.getMetadata()
        );
...
function findPartPaths(docxFile) {
    return readPackageRelationships(docxFile).then(function(packageRelationships) {
        ...
        return xmlFileReader({
            ...
            function findPartRelatedToDocProps(name) {
                return findPartPath({
                    docxFile: docxFile,
                    relationships: documentRelationships,
                    relationshipType: "http://schemas.openxmlformats.org/officeDocument/2006/relationships/" + name,
                    basePath: zipfile.splitPath(mainDocumentPath).dirname,
                    fallbackPath: "docProps/" + name + ".xml"
                });
            }

            return {
                mainDocument: mainDocumentPath,
                coreProps: findPartRelatedToDocProps("core"),
                appProps: findPartRelatedToDocProps("app"),
...
function readMetadataFromZipFile(zipFile, path) {
    return xmlFileReader({
        filename: path,
        readElement: metadataReader.readMetadataXml,
        defaultValue: metadataReader.defaultMetadata
    })(zipFile);
}
